...
func (r *AccountRepository) FindByAPIKey(apiKey string) (*domain.Account, error) {
	...
	err := r.db.QueryRow("SELECT id, name, email, api_key, balance, created_at, updated_at FROM accounts WHERE api_key = $1", apiKey).Scan(&account.ID, &account.Name, &account.Email, &account.APIKey, &account.Balance, &createdAt, &updatedAt)
...
func (r *AccountRepository) FindById(id string) (*domain.Account, error) {
	...
	err := r.db.QueryRow("SELECT id, name, email, api_key, balance, created_at, updated_at FROM accounts WHERE id = $1", id).Scan(&account.ID, &account.Name, &account.Email, &account.APIKey, &account.Balance, &createdAt, &updatedAt)
